...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // new node or on those existing nodes that are adjacent to the new node.
   ...
   Node* newNode =new Node(newData); // using constructors to construct new node and assign data.
   //std::cout<<"incoming data"<<newNode->data<<std::endl;
   if (!head_){ //if the list is empty then set List's head and tail to this new item
                //the node already has next and prev set to nullptr by default constructor
     head_=newNode;
     tail_=newNode;
     size_++;
     return;
   ...
   Node* thru_head = head_;
   Node* thru_tail = tail_;
   if (size_ ==1) {
    if( thru_head -> data <= newNode->data){
      // std::cout<<"pushback"<<std::endl; 
       thru_tail->next=newNode;
       newNode->prev=thru_tail;
       tail_=newNode;
      }
   else {   
    //std::cout<<"push front"<<std::endl; 
    thru_head->prev = newNode;
    newNode->next = thru_head;
    head_ = newNode;
    }
   size_++;
   return;
  ...
  // std::cout<<size_<<std::endl;
   //thru_head= thru_head->next;
   //std::cout<<"lolo"<<thru_head->next->data<<thru_head->data<<std::endl;
   while ( thru_head != nullptr) {
     //std::cout<<
     if (thru_head->data <=  newNode->data && thru_head->next != nullptr){
       // std::cout<<thru_head->data <<"comparison"  <<newNode->data<<std::endl;
        thru_head=thru_head->next;
        //std::cout<<thru_head->data<<" final data "<<std::endl;
      
     }
     else{
      //std::cout<<"lololol"<<thru_head->prev<<std::endl;
      //std::cout<<thru_head->data <<"comparison in else"  <<newNode->data<<std::endl;
      if(thru_head->prev == nullptr ){//if the next item after head is null
         //front
         //std::cout<<" front "<<std::endl;
         thru_head->prev=newNode; //
   	 newNode->next=thru_head;
    	 head_=newNode;
        // std::cout<<"after adding to front  "<<thru_head->prev->data<<std::endl;
         size_++;
         return;
      }
      if(thru_head->next == nullptr && thru_head->data <=newNode->data ){
        //std::cout<<" backside addition "<<std::endl;
        thru_head->next=newNode;
        newNode->prev=thru_head;
        tail_=newNode;
        size_++;
        return;      
      }         
      ...
      //std::cout<<thru_head->prev->data<<" middle insertion here "<<thru_head->data<<std::endl;
      Node* two_bore_thru=thru_head->prev;
      thru_head->prev=newNode;
      newNode->next=thru_head;
      newNode->prev=two_bore_thru;
      two_bore_thru->next=newNode;
      size_++;
      return;
     } 
  }   
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  //if size_
  //std::cout<<"this is the size of left"<<left.size_<<std::endl; 
  if (size_ < 1){
    if (right.size_<1){ return merged;}
    else if  (right.size_ >= 1){
     merged=right;
     return merged;
     }
   }
  ...
  const Node* thisCur=nullptr;
  const Node* otherCur=nullptr;
 
  if (left.size_ >= right.size_ ) {
     thisCur=left.head_;//left
     otherCur=right.head_; //right
   }
   else{
     thisCur=right.head_;//left
     otherCur=left.head_; //right
  }
 // std::cout<<"left"<<left<<std::endl;
 // std::cout<<"right"<<right<<std::endl;
 //while(thisCur){std::cout<<thisCur->data; thisCur=thisCur->next;} 
  while(thisCur!= nullptr){//means if not equal to nullptr
 //    std::cout<<thisCur->data<<std::endl;
     if(otherCur == nullptr){    
        while(thisCur){
 //          std::cout<<"in here"<<thisCur->data<<std::endl;
           merged.pushBack(thisCur->data);
           thisCur=thisCur->next;
      }
     
  //     std::cout<<"merged up"<<merged<<std::endl;
       return merged;  
     }
     if(thisCur->data <=otherCur->data){ 
        merged.pushBack(thisCur->data);
        thisCur=thisCur->next;
        if(thisCur == nullptr){merged.pushBack(otherCur->data);}
      }
      else if (thisCur->data >= otherCur->data){
 ...
 // std::cout<<"merged down"<<merged<<std::endl;
  return merged;
}
