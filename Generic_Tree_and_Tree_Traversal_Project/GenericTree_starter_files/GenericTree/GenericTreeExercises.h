...
static void treeFactory(GenericTree<int>& tree) {
  ...
  tree.createRoot(4);
  //nodeA = new TreeNode(4);
  auto nodefour = tree.getRootPtr();
  auto nodeeight=nodefour->addChild(8);
  nodeeight->addChild(16)->addChild(42);
  nodeeight->addChild(23);
  nodefour->addChild(15);
  //std::cout << "This is my print" <<tree << std::endl;
  
  //std::cout << "This is my tree" <<nodeA << std::endl;
}
  ...
  while (!nodesToExplore.empty()){
    ...
    // onto the exploration stack.
      for (auto& childPtr : frontNode->childrenPtrs) {
        //std::cout<<(childPtr->data)<<std::endl;
